...
export function LoadWorkModesTab() {
  ...
  const data = useMemo(() => {
    if (!activeATA) return rawData;
    return rawData.filter((w: any) => {
  ...
  const columns: Column<any>[] = [
    ...
    { title: '设备名称', dataIndex: 'equipment_name', key: 'name', width: 170,
      render: (v: string) => <span className="truncate block max-w-[160px]">{v || '-'}</span> },
    { title: '系统章节', dataIndex: 'ata_chapter', key: 'ata', width: 150,
      render: (v: string) => <span className="truncate block max-w-[140px]">{v || '-'}</span> },

...
function EquipmentVisualPreview({ equipment, allEquipment }: { equipment: Equipment; allEquipment: Equipment[] }) {
  ...
  const metrics = useMemo(() => {
    ...
    const sameVoltageTotal = allEquipment
      .filter(eq => eq.power_voltage === myVoltage && eq.electrical_load?.power_kva_normal)
      .reduce((s, eq) => s + (eq.electrical_load?.power_kva_normal ?? 0), 0);
    const myPower = e.electrical_load?.power_kva_normal ?? 0;

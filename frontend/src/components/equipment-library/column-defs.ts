...
export const ATTR_GROUPS: AttrGroup[] = [
  ...
  },
  {
    ...
    columns: [
      { key: 'part_number', label: '件号', mono: true, searchable: true, width: '115px' },
      { key: 'name', label: '设备类型名称', searchable: true },
      { key: 'internal_grounding', label: '共地', searchable: true },
      { key: 'shell_grounding_method', label: '壳体接地' },
      { key: 'shell_grounding_fault_path', label: '故障路径' },
      { key: 'grounding_special_requirements', label: '接地要求' },
      { key: 'grounding_terminal_diameter', label: '端子径' },
      { key: 'bonding_method', label: '搭接方式' },
      { key: 'bonding_type', label: '搭接类型' },
      { key: 'bonding_resistance', label: '阻值mΩ' },
    ],
  },
  {
